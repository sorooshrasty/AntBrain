"""Slide 5 verification with corrected centered feed-leg coordinates.

This version models the visible feed leg as the lumped-port/feed surface
starting at the dipole center gap.  It is centered at x=0 and starts at the
dipole centerline; it is not a PEC metal strip that shorts the dipole arms.

This is intended to match the Slide 5 visual interpretation where the feed
point is at the start of the centered feed leg.
"""
...
# Visual feed-leg surface.  It is the port object, not a PEC conductor.
FEED_LEG_LENGTH = 35.0e-3
...
def run_rapidfem() -> tuple[np.ndarray, np.ndarray, int, int]:
    print("Building slide-5 dipole with centered feed-leg port...")
    g = rf.Geometry(maxh=MAXH)

    domain_x = TOTAL_LENGTH + 2.0 * PAD
    domain_y = WIDTH + FEED_LEG_LENGTH + 2.0 * PAD
    domain_z = 2.0 * PAD
    air = g.box(
        domain_x,
        domain_y,
        domain_z,
        position=(-domain_x / 2.0, -FEED_LEG_LENGTH - PAD, -domain_z / 2.0),
        material=rf.Air(),
    )
    ...
    # Corrected feed coordinates:
    # x is centered across the 2 mm dipole gap; y starts at the dipole center
    # and extends outward.  The feed point is the start at y=0.
    feed_leg = g.xy_plate(
        GAP,
        FEED_LEG_LENGTH,
        position=(-GAP / 2.0, -FEED_LEG_LENGTH, 0.0),
        maxh=1.0e-3,
    )

    g.fragment(air, left_arm, right_arm, feed_leg)
    rf.LumpedPort(feed_leg, direction=(1, 0, 0), z0=50.0)
    rf.PEC(left_arm, right_arm)
...
def main() -> None:
    ...
    np.savetxt(
        OUT / "slide5_centered_feedleg_comparison.csv",
        np.column_stack((fem_f, fem_s11, ref_interp, err)),
        delimiter=",",
        header="frequency_hz,rapidfem_centered_feedleg_s11_db,slide5_reference_s11_db,error_db",
        comments="",
    )

    fig, ax = plt.subplots(figsize=(9, 5.4))
    ax.plot(ref_f / 1e9, ref_s11, color="#d62728", lw=2.0, label="Slide 5 reference")
    ax.plot(fem_f / 1e9, fem_s11, "o-", color="#1f77b4", lw=2.0, ms=4.5, label="RapidFEM centered feed-leg port")
    ax.axhline(-10.0, color="0.65", ls=":", lw=1)
    ax.set(
        xlabel="Frequency (GHz)",
        ylabel=r"$|S_{11}|$ (dB)",
        title=(
            "Slide 5 verification: corrected centered feed-leg coordinate\n"
            f"feed starts at dipole center, leg={FEED_LEG_LENGTH*1e3:.0f} mm; RMS difference: {rms_err:.2f} dB"
        ),
        xlim=(0.5, 1.5),
        ylim=(-16, 0),
    )
    ...
    fig.tight_layout()
    fig.savefig(OUT / "slide5_centered_feedleg_comparison.png", dpi=180)
    plt.close(fig)

    print(f"RapidFEM mesh: {n_tets} tets / {n_dofs} DOFs")
    print(f"Feed point coordinate: x=0 mm, y=0 mm, z=0 mm")
    print(f"Centered feed leg: width={GAP*1e3:.1f} mm, length={FEED_LEG_LENGTH*1e3:.1f} mm")
    print(f"Slide reference min: {ref_s11[ref_best_idx]:.2f} dB at {ref_f[ref_best_idx]/1e9:.3f} GHz")
    print(f"RapidFEM centered-feedleg min: {fem_s11[fem_best_idx]:.2f} dB at {fem_f[fem_best_idx]/1e9:.3f} GHz")
    print(f"RMS difference at FEM points: {rms_err:.2f} dB")
    print(f"Wrote {OUT / 'slide5_centered_feedleg_comparison.png'}")
    print(f"Wrote {OUT / 'slide5_centered_feedleg_comparison.csv'}")
