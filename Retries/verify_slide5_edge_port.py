"""Slide 5 verification with edge-connected lumped feed port.

Geometry used in this run:

* dipole length: 165 mm
* dipole strip width: 4 mm
* center gap: 2 mm
* two parallel feed metals, clear spacing = 2 mm
* feed-metal length: 35 mm
* feed/port at the lower feed-metal end
* lumped port spans edge-to-edge across the 2 mm gap and touches the inner
  metal edges, rather than being interpreted as only a center point
"""
...
def run_rapidfem() -> tuple[np.ndarray, np.ndarray, int, int]:
    print("Building Slide 5 edge-connected feed-port geometry...")
    ...
    # Edge-connected port: starts at y=-35 mm and extends slightly upward,
    # so its left/right edges coincide with the inner feed-metal edges.
...
def main() -> None:
    ...
    np.savetxt(
        OUT / "slide5_edge_port_comparison.csv",
        np.column_stack((fem_f, fem_s11, ref_interp, err)),
        delimiter=",",
        header="frequency_hz,rapidfem_edge_port_s11_db,slide5_reference_s11_db,error_db",
        comments="",
    )

    fig, ax = plt.subplots(figsize=(9, 5.4))
    ax.plot(ref_f / 1e9, ref_s11, color="#d62728", lw=2.0, label="Slide 5 reference")
    ax.plot(fem_f / 1e9, fem_s11, "o-", color="#1f77b4", lw=2.0, ms=4.5, label="RapidFEM edge-connected port")
    ax.axhline(-10.0, color="0.65", ls=":", lw=1)
    ax.set(
        xlabel="Frequency (GHz)",
        ylabel=r"$|S_{11}|$ (dB)",
        title=(
            "Slide 5 verification: edge-connected 50 Ω lumped port\n"
            f"feed at y=-35 mm, port touches metal edges; RMS difference: {rms_err:.2f} dB"
        ),
        xlim=(0.5, 1.5),
        ylim=(-16, 0),
    )
    ...
    fig.tight_layout()
    fig.savefig(OUT / "slide5_edge_port_comparison.png", dpi=180)
    plt.close(fig)
    ...
    print(f"Edge-connected port: x gap={CENTER_GAP*1e3:.1f} mm, y=[{-FEED_LENGTH*1e3:.1f}, {(-FEED_LENGTH + PORT_DEPTH)*1e3:.1f}] mm")
    print(f"Slide reference min: {ref_s11[ref_best_idx]:.2f} dB at {ref_f[ref_best_idx]/1e9:.3f} GHz")
    print(f"RapidFEM edge-port min: {fem_s11[fem_best_idx]:.2f} dB at {fem_f[fem_best_idx]/1e9:.3f} GHz")
    print(f"RMS difference at FEM points: {rms_err:.2f} dB")
    print(f"Wrote {OUT / 'slide5_edge_port_comparison.png'}")
    print(f"Wrote {OUT / 'slide5_edge_port_comparison.csv'}")
